...
export default function Home() {
  ...
  const { isLoading, error, data, refetch } = useQuery<WeatherData>("repoData", async () => {
    const { data } = await axios.get(
      `${process.env.NEXT_PUBLIC_WEATHER_URL}/forecast?q=${place}&appid=${process.env.NEXT_PUBLIC_WEATHER_KEY}&cnt=56`
    )
    return data
  })
  ...
  return (
    <div className="flex flex-col gap-4 bg-[#7bedfe] min-h-screen ">
